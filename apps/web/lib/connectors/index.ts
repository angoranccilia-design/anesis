...
const contract = (c: Omit<ConnectorContract, "status" | "statusNote" | "lastFetchedAt" | "fields"> & Partial<Pick<ConnectorContract, "status" | "statusNote" | "lastFetchedAt" | "fields">>): ConnectorContract => {
  const missing = c.requires.filter((k) => !process.env[k]);
  const status: ConnectorContract["status"] = c.requires.length ? (missing.length ? "AUTHENTICATION_REQUIRED" : "NOT_CONNECTED") : "NOT_CONNECTED";
  return { status, statusNote: c.requires.length ? (missing.length ? `credentials required: ${missing.join(", ")} not set; no data is fabricated` : "credentials present; adapter for this provider not yet implemented") : "no provider contract in this environment", lastFetchedAt: null, fields: [], ...c };
};
...
export async function fetchWeather(spec: PropertySpec): Promise<{ contract: ConnectorContract; data: WeatherForecast | null }> {
  const base = contract({ id: "CONN-WEATHER", fields: ["date", "precipitation", "temperature max/min", "wind", "sunshine"], domain: "weather", name: "Weather forecast", provider: "Open-Meteo (open data, no key)", kind: "live", requires: [], freshness: "DAILY", legal: "Open-Meteo non-commercial/attribution terms; commercial use needs the paid tier" });
  try {
  ...
  } catch (e) { return { contract: { ...base, status: "DATA_UNAVAILABLE", statusNote: `fetch failed: ${e instanceof Error ? e.message : String(e)}; no value is fabricated` }, data: null }; }
}

export async function fetchHolidays(country: string): Promise<{ contract: ConnectorContract; data: Holiday[] }> {
  const base = contract({ id: "CONN-CALENDAR", fields: ["date", "name", "region"], domain: "calendar", name: "Public holidays", provider: country === "GB" ? "gov.uk bank holidays (open data)" : "Nager.Date (open data)", kind: "live", requires: [], freshness: "STATIC", legal: "open government data" });
  try {
    if (country === "GB") {
  ...
  } catch (e) { return { contract: { ...base, status: "DATA_UNAVAILABLE", statusNote: `fetch failed: ${e instanceof Error ? e.message : String(e)}; no value is fabricated` }, data: [] }; }
}

export async function fetchFx(): Promise<{ contract: ConnectorContract; data: MacroIndicator[] }> {
  const base = contract({ id: "CONN-FX", fields: ["base", "quote", "rate", "date"], domain: "macro", name: "Exchange rates (GBP)", provider: "Frankfurter (ECB reference rates, no key)", kind: "live", requires: [], freshness: "DAILY", legal: "public ECB reference rates" });
  try {
...
export const CONTRACT_ONLY: ConnectorContract[] = [
  contract({ id: "CONN-PMS", fields: ["reservations", "room nights", "ADR", "occupancy", "cancellations", "no-shows", "booking window", "source/channel", "revenue", "inventory"], domain: "pms", name: "Property management system", provider: "Mews / Opera Cloud / Guestline (API)", kind: "contract", requires: ["ANESIS_PMS_PROVIDER", "ANESIS_PMS_API_KEY", "ANESIS_PMS_PROPERTY_ID"], freshness: "HOURLY", legal: "property's own data under its PMS contract; read-only scope" }),
  contract({ id: "CONN-BOOKING-ENGINE", fields: ["searches", "availability checks", "booking starts", "booking completions", "abandonment", "device", "package/treatment", "timestamps"], domain: "booking_engine", name: "Booking engine", provider: "SynXis / SiteMinder / Profitroom (API)", kind: "contract", requires: ["ANESIS_BE_PROVIDER", "ANESIS_BE_API_KEY"], freshness: "HOURLY", legal: "property's own data" }),
  contract({ id: "CONN-GA4", fields: ["sessions", "users", "landing pages", "engagement", "events", "conversions", "device", "source/medium", "campaign"], domain: "web_analytics", name: "Google Analytics 4", provider: "GA4 Data API (service account)", kind: "contract", requires: ["ANESIS_GA4_PROPERTY_ID", "GOOGLE_APPLICATION_CREDENTIALS"], freshness: "DAILY", legal: "property grants viewer access" }),
  contract({ id: "CONN-GOOGLE-ADS", fields: ["spend", "impressions", "clicks", "CPC", "conversions", "conversion value where available", "campaign", "date"], domain: "google_ads", name: "Google Ads", provider: "Google Ads API", kind: "contract", requires: ["ANESIS_GADS_CUSTOMER_ID", "ANESIS_GADS_DEVELOPER_TOKEN", "ANESIS_GADS_REFRESH_TOKEN"], freshness: "DAILY", legal: "manager-account link approved by the property" }),
  contract({ id: "CONN-META-ADS", fields: ["spend", "impressions", "clicks", "leads", "conversions", "conversion value where available", "campaign", "date"], domain: "meta_ads", name: "Meta Ads", provider: "Marketing API", kind: "contract", requires: ["ANESIS_META_AD_ACCOUNT_ID", "ANESIS_META_ACCESS_TOKEN"], freshness: "DAILY", legal: "partner access to the ad account" }),
  contract({ id: "CONN-CRM", fields: ["contacts", "lifecycle", "repeat booking", "retention", "email activity", "upsell", "cancellation", "customer value"], domain: "crm", name: "Guest CRM", provider: "Revinate / For-Sight / HubSpot (API)", kind: "contract", requires: ["ANESIS_CRM_PROVIDER", "ANESIS_CRM_API_KEY"], freshness: "DAILY", legal: "aggregates only; no guest-level export without a data-processing agreement" }),
  contract({ id: "CONN-RMS", fields: ["ADR", "forecast occupancy", "rate recommendations", "pickup"], domain: "rms", name: "Revenue management system", provider: "IDeaS / Duetto / Atomize (API)", kind: "contract", requires: ["ANESIS_RMS_PROVIDER", "ANESIS_RMS_API_KEY"], freshness: "DAILY", legal: "read-only forecast and rate export" }),
  contract({ id: "CONN-OTA", fields: ["channel", "bookings", "revenue", "commission", "cancellation", "availability", "ranking/visibility where legitimately accessible"], domain: "ota", name: "OTA channel data", provider: "Booking.com / Expedia / Airbnb via channel manager", kind: "contract", requires: ["ANESIS_CHANNEL_MANAGER_API_KEY"], freshness: "WEEKLY", legal: "property's own extranet data through its channel manager; no scraping" }),
  contract({ id: "CONN-EVENTS", fields: ["name", "dates", "venue", "distance", "expected attendance", "category"], domain: "events", name: "Future events", provider: "PredictHQ / Ticketmaster Discovery (API) or operator register", kind: "contract", requires: ["ANESIS_EVENTS_PROVIDER", "ANESIS_EVENTS_API_KEY"], freshness: "WEEKLY", legal: "provider terms; operator-entered events are accepted with their evidence note", statusNote: "no provider key; operator-entered events are used when present" }),
  contract({ id: "CONN-SEARCH", fields: ["term", "weekly index", "geography", "branded vs non-branded"], domain: "search", name: "Search demand", provider: "Google Trends (no official API); Google Search Console for branded queries", kind: "contract", requires: ["ANESIS_GSC_SITE_URL", "GOOGLE_APPLICATION_CREDENTIALS"], freshness: "WEEKLY", legal: "Trends has no official API and its terms prohibit scraping; Search Console needs the property's consent" }),
  contract({ id: "CONN-INFLATION", fields: ["CPIH index", "month"], domain: "macro", name: "UK inflation (CPIH)", provider: "ONS API (beta)", kind: "live", requires: [], freshness: "MONTHLY", legal: "open government data", statusNote: "endpoint returned HTTP 500 on the observations query on 24 September 2026; contract kept, no value invented" }),
  contract({ id: "CONN-BANK-RATE", fields: ["bank rate", "effective date"], domain: "macro", name: "Bank of England base rate", provider: "Bank of England database", kind: "live", requires: [], freshness: "MONTHLY", legal: "open data", statusNote: "CSV endpoint returns an HTML consent page from this environment; contract kept, no value invented" }),
  contract({ id: "CONN-COMPETITOR", fields: ["competitor", "date", "rate", "availability", "review score", "review velocity"], domain: "competitor", name: "Competitor rates and visibility", provider: "Lighthouse / OTA Insight (rate shopping, contract) or operator-entered observations", kind: "contract", requires: ["ANESIS_RATESHOP_PROVIDER", "ANESIS_RATESHOP_API_KEY"], freshness: "DAILY", legal: "no scraping of OTA or competitor sites; licensed rate-shopping data or manual observations only", statusNote: "no rate-shopping licence; operator-entered observations are used when present" }),
];

export function mapsConnector(spec: PropertySpec): ConnectorContract {
  return contract({ id: "CONN-MAPS", fields: ["name", "latitude", "longitude", "country"], domain: "maps", name: "Geographic data", provider: "Open-Meteo geocoding (open data)", kind: "live", requires: [], freshness: "STATIC", legal: "open data", status: "CONNECTED", statusNote: `property located at ${spec.location.name} (${spec.location.lat}, ${spec.location.lon}); geocoding endpoint reachable`, lastFetchedAt: new Date().toISOString() });
}
...
export async function buildContext(spec: PropertySpec = COUNTRY_HOUSE_34, reg: OperatorRegisters = emptyRegisters(), overrides: { weather?: WeatherForecast | null; offline?: boolean } = {}): Promise<ExternalContext> {
  ...
  const offlineNote = (id: string, domain: ConnectorContract["domain"], name: string, provider: string, fresh: Freshness): ConnectorContract => contract({ id, domain, name, provider, kind: "live", requires: [], fields: [], freshness: fresh, legal: "open data", status: "NOT_CONNECTED", statusNote: "offline mode requested: live fetch skipped" });
